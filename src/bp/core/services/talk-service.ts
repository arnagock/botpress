...
// TODO: Maybe find another name than "Talk"
...
export class TalkService {
  ...
  async init() {
    this.eventEngine.register({
      name: 'json.api.captureMessages',
      description: 'Capture messages that comes from the json api channel',
      direction: 'outgoing',
      handler: (event: sdk.IO.Event, next) => console.log('OUT', event),
      order: 2
    })
  }

  outgoingHandler = (event: sdk.IO.Event, next) => {
    console.log('FROM BOT', event)
    if (event.channel === 'api.json') {
    }
    next()
  }
  ...
  async sendNewMessage(botId: string, userId: string, payload) {
    ...
    // Long polling but got to wait for outgoing
  }
